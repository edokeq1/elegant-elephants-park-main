...
export const CascadeLakesSection: React.FC<CascadeLakesSectionProps> = ({ t }) => {
    ...
    return (
        <section ref={sectionRef} className="relative w-full py-24 md:py-32 px-6 md:px-12 bg-brand-dark border-t border-white/5 text-white overflow-hidden z-10">
            {/* Background Decor - Blue/Cyan for lakes/ice */}
            <div className="absolute top-0 right-0 w-1/2 h-1/2 bg-cyan-500/5 blur-3xl rounded-full translate-x-1/2 -translate-y-1/2 pointer-events-none" />
            ...
            <div className="max-w-[1800px] mx-auto relative z-10">
                ...
                <div className="grid grid-cols-1 lg:grid-cols-2 gap-16 lg:gap-24">
                    ...
                    <div className="space-y-12">
                        ...
                        {/* Cadastral Link - Only show if link exists/is provided */}
                        {t.cadastralLink && (
                            <div className={`flex flex-col sm:flex-row gap-4 transition-all duration-1000 delay-[600ms] transform ${isVisible ? 'translate-y-0 opacity-100' : 'translate-y-10 opacity-0'}`}>
                                <a
                                    href={t.cadastralLink} // Should be dynamic if provided
                                    target="_blank"
                                ...
                                <div className="grid grid-cols-1 sm:grid-cols-2 gap-2">
                                    {t.housingList.items.map((item: any, i: number) => (
                                        <div key={i} className="flex flex-col justify-center p-3 bg-white/5 rounded-xl border border-white/5 hover:bg-white/10 transition-colors shadow-lg shadow-black/20">
                                            ...
                                            <span className="text-gray-500 text-[9px] uppercase tracking-wider">{item.area}</span>
                                        </div>
                                    ))}
                                </div>
                            </div>
